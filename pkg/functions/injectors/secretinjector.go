...
func getSecrets(ctx context.Context, c client.SecretsClient, secretNames []string) (map[string]interface{}, error) {

	secrets := make(map[string]interface{})
	for _, name := range secretNames {
		resp, err := c.GetSecret(ctx, name)
		if err != nil {
...
func (i *secretInjector) GetMiddleware(secretNames []string, cookie string) functions.Middleware {
	return func(f functions.Runnable) functions.Runnable {
		return func(ctx functions.Context, in interface{}) (interface{}, error) {
			...
			if span, ok := ctx[functions.TracingKey]; ok {
				localCtx = opentracing.ContextWithSpan(localCtx, span.(opentracing.Span))
			}
			secrets, err := getSecrets(localCtx, i.secretClient, secretNames)
			if err != nil {
